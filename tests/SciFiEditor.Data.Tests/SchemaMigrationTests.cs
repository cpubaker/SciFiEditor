...
public class SchemaMigrationTests
{
    ...
    public void EnsureSchema_AddsMissingColumns_ToAnM1ShapedTable_WithoutDataLoss()
    {
        ...
        node.IncludeInCompile.Should().BeTrue();
    }

    [Fact]
    public void EnsureSchema_AddsIncludeInCompileColumn_ToAnM2ShapedTable_WithoutDataLoss()
    {
        using var temp = new TempDirectory();
        var dbPath = Path.Combine(temp.Path, ProjectDatabase.DatabaseFileName);
        var nodeId = Guid.NewGuid();

        // Simulate a project.db created by the M2 schema, before the M4 include_in_compile column existed.
        using (var connection = new SqliteConnection($"Data Source={dbPath};Pooling=False"))
        {
            connection.Open();
            using var create = connection.CreateCommand();
            ...
            using var insert = connection.CreateCommand();
            insert.CommandText = """
                INSERT INTO nodes (id, parent_id, node_type, title, sort_order, is_trashed, original_parent_id, created_at_utc, updated_at_utc, word_count, char_count)
                VALUES (@Id, NULL, 'Scene', 'M2-era Scene', 1000, 0, NULL, '2026-01-01T00:00:00.0000000Z', '2026-01-01T00:00:00.0000000Z', 500, 2500)
                """;
            insert.Parameters.AddWithValue("@Id", nodeId.ToString());
            insert.ExecuteNonQuery();
        }
        ...
        node.Should().NotBeNull();
        node!.Title.Should().Be("M2-era Scene");
        node.WordCount.Should().Be(500);
        node.IncludeInCompile.Should().BeTrue();
    }
